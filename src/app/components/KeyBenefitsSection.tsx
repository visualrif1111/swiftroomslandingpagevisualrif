  ...
  return (
    ...
    >
      <div className="container mx-auto px-5 lg:px-6 relative z-10">
        ...
        {/* Benefit Cards — spacious rounded rows on mobile, grid on desktop */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 lg:gap-6 max-w-6xl mx-auto">
          {benefits.map((benefit, index) => {
            const Icon = benefit.icon;
            return (
              <motion.div
                key={benefit.title}
                className="group bg-white sr-card-premium flex flex-row lg:flex-col items-start gap-4 lg:gap-0 p-5 lg:p-6 lg:rounded-2xl lg:shadow-none lg:border lg:border-[#e5e7eb] hover:border-[#007969] lg:card-hover"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: '-30px' }}
                transition={{ duration: 0.5, delay: index * 0.06 }}
              >
                <div className="shrink-0 w-12 h-12 lg:w-14 lg:h-14 rounded-2xl lg:rounded-xl bg-[#007969]/10 flex items-center justify-center lg:mb-4 group-hover:bg-[#007969] transition-colors duration-300">
                  <Icon className="w-6 h-6 lg:w-7 lg:h-7 text-[#007969] group-hover:text-white transition-colors duration-300" strokeWidth={2} />
                </div>
                <div className="min-w-0">
                  <h3 className="font-heading text-[1.0625rem] lg:text-xl font-semibold text-[#1c1c1e] mb-1.5 lg:mb-2 leading-snug">
                    {benefit.title}
                  </h3>
